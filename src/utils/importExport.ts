...
export type ImportType = 'full' | 'section' | 'cards';

export async function importSubjectJSON(importType: ImportType = 'full', targetSubjectId?: number, targetSectionId?: number) {
  try {
    ...
    if (importType === 'full') {
      // Import entire subject with all sections
      await importFullSubject(data);
    } else if (importType === 'section') {
      // Import a single section into existing subject
      if (!targetSubjectId) {
        Alert.alert('Error', 'Target subject required for section import');
        return;
      }
      await importSection(data, targetSubjectId);
    } else if (importType === 'cards') {
      // Import only cards into existing section
      if (!targetSectionId) {
        Alert.alert('Error', 'Target section required for cards import');
        return;
      }
      await importCardsToSection(data, targetSectionId);
    }
...
async function importFullSubject(data: any) {
...
async function importSection(data: any, targetSubjectId: number) {
  db.withTransactionSync(() => {
    for (const section of data.sections) {
      const secRes = db.runSync('INSERT INTO sections (subject_id, name) VALUES (?, ?)', [targetSubjectId, section.name]);
      const sectionId = secRes.lastInsertRowId;
  ...
  Alert.alert('Success', 'Section(s) imported successfully');
}

async function importCardsToSection(data: any, targetSectionId: number) {
  db.withTransactionSync(() => {
    for (const section of data.sections) {
      if (section.cards) {
        for (const card of section.cards) {
          const cardRes = db.runSync('INSERT INTO cards (section_id, front, back) VALUES (?, ?, ?)', [targetSectionId, card.front, card.back]);
          const cardId = cardRes.lastInsertRowId;
          db.runSync('INSERT INTO card_progress (card_id, box, next_review) VALUES (?, 1, date("now"))', [cardId]);
        }
      }
  ...
  Alert.alert('Success', 'Cards imported successfully');
}
